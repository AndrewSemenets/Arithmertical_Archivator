...
void encode_symbol(int symbol)
{
	...
	for (;;)										/* Цикл по выводу битов  */
	{
		if (high < HALF)							/* Если в нижней половине*/
			output_bit_plus_follow(0);				/* исходного интеpвала,  */
													/* то вывод 0            */
		else if (low >= HALF)						/* Если в веpхней, то    */	
		{											/* вывод 1, а затем      */
			output_bit_plus_follow(1);				/* убpать известную у    */
			low -= HALF;							/* гpаниц общую часть    */
			high -= HALF;
		}
		else if (low >= FIRST_QTR && high < THIRD_QTR)	/* Если текущий интеpвал */
		...
		//cout << "high = " << high << " low = " << low << endl;
		low = 2 * low;								/* Расшиpить текущий pа-бочий кодовый интеpвал*/
		high = 2 * high + 1;
	}
}
...
int main(int argc, char *argv[])
{
	if (argc < 4)
	{
		printf("\n Bad arguments \nTry: ArithTest e or d infile outfile\n");
	}
	if (argv[1][0] == 'e')
